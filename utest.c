...
#include "uAVS3lib/uAVS3lib.h"

#define TEST_SPEED 0
#define USE_CLOSE_GOP 0
...
void (*write_frame)(int fd, image_t *out_img, int img_width, int img_height);

cfg_param_t input;
int fd_bitstream = 0;
int fd_rec = 0;
long long seq_start_idx = 0;
int drop_repeated_i = 0;
...
void write_frame_8bit(int fd, image_t *out_img, int img_width, int img_height)
{
    int j;
    ...
    int add = (1 << shift_bits) >> 1;
    int k;

    i_rec = out_img->i_stride[0];
    rec = out_img->plane[0];

    for (j = 0; j < img_height; j++) {
        for (k = 0; k < img_width; k++) {
            rec_buf[j * img_width + k] = (unsigned char)min(((rec[k] + add) >> shift_bits), 255);
        }
        rec += i_rec;
    }

    _write(fd, rec_buf, img_height * img_width);

    i_rec = out_img->i_stride[1];
    rec = out_img->plane[1];
    for (j = 0; j < img_height_cr; j++) {
        for (k = 0; k < img_width_cr; k++) {
            rec_buf[j * img_width_cr + k] = (unsigned char)min(((rec[k] + add) >> shift_bits), 255);
        }
        rec += i_rec;
    }

    _write(fd, rec_buf, img_height_cr * img_width_cr);

    i_rec = out_img->i_stride[2];
    rec = out_img->plane[2];
    for (j = 0; j < img_height_cr; j++) {
        for (k = 0; k < img_width_cr; k++) {
            rec_buf[j * img_width_cr + k] = (unsigned char)min(((rec[k] + add) >> shift_bits), 255);
...
void write_frame_10bit(int fd, image_t *out_img, int img_width, int img_height)
{
    int j;
    int img_width_cr = (img_width / 2);
    int img_height_cr = (img_height / 2);
...
void callback_output_rec(void *priv_data, image_t *rec, image_t *org, image_t *trace, int frm_type, long long idx, int bits, double qp, int time, signed char* ext_info)
{
    ...
    if (fd_rec) {
        int mul = input.sample_bit_depth == 8 ? 1 : 2;
        _lseeki64(fd_rec, rec->width * rec->height * 3 / 2 * idx * mul, SEEK_SET);
        if (trace) {
            write_frame(fd_rec, trace, trace->width, trace->height);
        }
        else {
            write_frame(fd_rec, rec, rec->width, rec->height);
        }
    }
...
#endif
{
...
#if 1
    handle = avs3_lib_create(&input, callback_output_bitstream, callback_output_rec, 1, NULL);
#else
    handle = avs3_lib_create(&input, NULL, NULL, input.succ_bfrms + 1 + input.threads_frm + 16, NULL);
#endif

    shift_bits = input.bit_depth - 8;

    rec_buf = malloc(input.img_width * input.img_height * 3 / 2);
    ...
    if (strlen(input.ReconFile) > 0 && (fd_rec = _open(input.ReconFile, O_WRONLY | O_CREAT | O_BINARY | O_TRUNC, S_IREAD | S_IWRITE)) <= 0) {
        printf("Error open file %s", input.ReconFile);
    }
    ...
    if (input.sample_bit_depth == 8) {
        ...
        write_frame = write_frame_8bit;
    } else {
        ReadOneFrame = ReadOneFrame_10bit;
        write_frame = write_frame_10bit;
    }

    avs3_lib_set_rc(handle, NULL, NULL, NULL, NULL);

    avs3_lib_set_priority(handle, 7);
    //avs3_lib_speed_adjust(handle, 5);

    int repeat = 0;

    for (i = 0; i < input.no_frames; i++) {
        if (!ReadOneFrame(avs3_lib_imgbuf(handle), fd_in, &input, i)) {
            if (repeat--) {
                i = 0;
                ReadOneFrame(avs3_lib_imgbuf(handle), fd_in, &input, i);
            } else {
                break;
            }
        }
        avs3_lib_encode(handle, 0, 0);

        total_frms++;

#if USE_CLOSE_GOP
        if (total_frms % (input.gop_size * input.intra_period) == 0) {
            avs3_lib_encode(handle, 1, 1);
            avs3_lib_reset(handle, &input, NULL);
            seq_start_idx = total_frms;
        }
#endif
    }
